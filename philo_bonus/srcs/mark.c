...
/*   Updated: 2026/02/18 15:58:12 by sshimots         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	try_mark_eating(t_philo *philo)
{
	...
	{
		printf("%lld\t%d\t%s\n", utom(philo->last_eat_time - philo->start_time), philo->id + 1, "died");
		trigger_termination(philo->sem->stop);
		wait_forever();
	}
	printf("%lld\t%d\t%s\n", utom(philo->last_eat_time - philo->start_time), philo->id + 1, "is eating");
