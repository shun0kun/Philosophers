...
- ダブルフリー問題。ダブルクローズ問題。残るはこれだけ。semで済ませちゃうか、設計ごと変えるか。残りは結構良いのではないかと思う。明日仕上げる。
- get_num_len()まだ実装してないからやる。
- 提出前に念入りにチェックする。可能であれば誰かにチェックしてもらう。

- time_to_sleepが0 かつ 偶数人のときに、同じ人が資源を取り続けてる。→ ./philo 2 1000 400 0でも飢餓してる！！！！！！！！！！！！！！！！　→　解決した。

- ./philo 2 119 60 60 のときに死なないで満腹になってる！ →　至って正常な挙動でした。

- pthread_create()のエラーハンドリングをしてもいい。

- 内部時間はすべてusで行う。出力だけmsにする。
...
void	my_pid(void)
{
	...
	fd = open("pid", O_WRONLY | O_CREAT | O_TRUNC, 0644);
	dprintf(fd, "PID=%d\n", getpid());
	close(fd);
}
```